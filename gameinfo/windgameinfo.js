// 클래스명 바꿔주기
export default class WindGameInfo{
  ...
  draw(ctx){
    ...
    ctx.drawImage(this.img, 0, 0, this.img.width, this.img.height, 10, 150, 400, 500);    
...
printSubtitle(ctx) {
  ...
  ctx.fillText("늑대 입김 불기 게임 도움말", 410, 80);
  
  ctx.font = '30pt Jua';
  ctx.fillStyle = `rgba(0,0,0,${this.fontlight})`; 
  ctx.fillText("늑대가 입김을 모으고 있어요!", 450, 250);
  ctx.fillText("스페이스바를 연속해서 누르면", 450, 380);
  ctx.fillText("늑대가 얼마나 입김을 모으는지 볼 수 있어요.", 450, 430);
  ctx.fillText("아기 돼지의 집이 얼마나 튼튼한지 확인해볼까요?", 450, 550);

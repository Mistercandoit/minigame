// 클래스명 바꿔주기
export default class Wolfpig1talk {
  constructor() {
    // 이미지 파일들 불러오는 부분
    ...
    this.img = new Image();
    this.img.src = '../img/S6.jpg';

    // 오디오 파일 불러오기 (src 임시) , .loop 자동반복 false로 막음
    this.audio = new Audio();
    this.audio.src = '../audio/sound/scene6.mp3';
    ...
    this.subtitleX = 800;
    this.subtitleY = 500; /** 자막 위치 */
    this.lineInterval = 60; /** 자막 간격 */
    this.subtitleTxt = [
      '놀란 첫째 돼지는 문을 꼭 붙잡고선 말했어요.',
      '"안 돼! 절대로 열어줄 수 없어!"',
      '그러자 늑대는 입김을 모으기 시작했어요.',
    ]; /*여기다가 멘트들 한줄씩 넣으면 됨*/
  ...
  printSubtitle(ctx) {
    ...
    ctx.fillText('둘을 쫓던 늑대는 먼저 첫째 돼지 집으로 갔어요.',50,50);
    ctx.fillText('"꼬마 돼지야, 나 좀 들여보내줘"',50,100);    

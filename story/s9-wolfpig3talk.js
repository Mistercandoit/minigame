// 클래스명 바꿔주기
export default class WolfPig3Talk{
  ...
  constructor() {
    ...
    this.wolfImg = document.querySelector('#w-bg-wolf');
    ...
    this.img = new Image();
    this.img.src = "img/S9.jpg";
    
    // 오디오 파일 불러오기 (src 임시) , .loop 자동반복 false로 막음
    this.audio = new Audio();
    this.audio.src = "audio/sound/scene9.mp3";
    ...
    this.subtitleX = 800;
    this.subtitleY = 70; /** 자막 위치 */
    this.lineInterval = 40; /** 자막 간격 */
    this.subtitleTxt = ['첫째, 둘째 돼지는 결국 막내',
                        '돼지 집으로 도망쳐왔어요.',
                        '배고픈 늑대는 또 다시 쫓아와 물었어요.',
                        '"꼬마 돼지들아, 나 좀 들여보내줘"',
                        '셋째 돼지는 문을 잡고는 말했어요.',
                        '"안 돼! 열어줄 수 없어!"',
                        '그러자 이번에도 늑대는 입김을 모으기 시작했어요.']; /*여기다가 멘트들 한줄씩 넣으면 됨*/
  ...
  draw(ctx){
    ...
    if((this.fwdbtnx<this.onmovex && this.onmovex < 1400) && (this.fwdbtny<this.onmovey && this.onmovey < 100)) {
      ctx.drawImage(this.wolfImg, 0, 0, this.wolfImg.width, this.wolfImg.height, this.fwdbtnx-50, this.fwdbtny, this.dirBtnSz+50, this.dirBtnSz+50);
    }
    else
      ctx.drawImage(this.gameBtn, 0, 0, this.forwardBtn.width, this.forwardBtn.height,this.fwdbtnx, this.fwdbtny, this.dirBtnSz, this.dirBtnSz);    

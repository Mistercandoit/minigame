// 클래스명 바꿔주기
export default class StoryInfo{
  ...
  draw(ctx){
    ...
    ctx.drawImage(this.forwardBtn, 0, 0, this.forwardBtn.width, this.forwardBtn.height, 100, 170, this.dirBtnSz, this.dirBtnSz);
    ctx.drawImage(this.gameBtn, 0, 0, this.forwardBtn.width, this.forwardBtn.height, 100, 300, this.dirBtnSz, this.dirBtnSz);
...
printSubtitle(ctx) {
  ...
  ctx.fillText("동화 진행 도움말", 460, 80);
  
  ctx.font = '30pt Jua';
  ctx.fillStyle = `rgba(0,0,0,${this.fontlight})`; 
  ctx.fillText("오른쪽 상단에 버튼을 통해 다음 이야기를 진행할 수 있어요.", 300, 230);
  ctx.fillText("버튼 색이 파랑색이면 다음 페이지에는 게임이 진행되요.", 300, 360);
  ctx.fillText("아래 버튼들은 각각 재생, 일시정지, 다시듣기 버튼이에요.", 300, 480);
  ctx.fillText("재생 버튼을 클릭하면 동화 음성이 재생되요.", 300, 530);
